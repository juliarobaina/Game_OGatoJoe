...
int venceuTela(){
	...
	const Uint8 *state = SDL_GetKeyboardState(NULL);
			...
			jogarnovamenteRect.x = (LARGURA_JANELA) - (jogarnovamenteSurface->w) - 70;
			...
			inputRect.x = (LARGURA_JANELA/2) - (inputSurface->w/2) -20;
			inputRect.y = (ALTURA_JANELA) - inputSurface->h - 130;//(ALTURA_JANELA/2) -100;
						...
						switch(event.type){
							case SDL_MOUSEMOTION:
								...
								if(x >= jogarnovamenteRect.x && x <= jogarnovamenteRect.x+jogarnovamenteRect.w && y >= jogarnovamenteRect.y && y <= jogarnovamenteRect.y+jogarnovamenteRect.h){
										...
										jogarnovamenteTexto = SDL_CreateTextureFromSurface(renderer,jogarnovamenteSurface);
										pataRect.x = (LARGURA_JANELA) - (jogarnovamenteSurface->w) - 112;
										//jogarnovamenteSurface->w/2 - voltarSurface->w + ((LARGURA_JANELA/2) - (jogarnovamenteSurface->w));
										pataRect.y = (ALTURA_JANELA - 16) - (voltarSurface->w/2 + voltarSurface->h/2);
