void fading(SDL_Renderer *renderer){
    ...
    SDL_SetTextureBlendMode(pretoFade, SDL_BLENDMODE_BLEND);
    SDL_SetTextureAlphaMod(pretoFade, fadingAlpha);
    SDL_RenderCopy(renderer, pretoFade, NULL, NULL);
}
int fase3(){
...
int creditos(){
	...
	if(backgroundTextura != NULL){
		SDL_DestroyTexture(backgroundTextura);
		backgroundTextura = NULL;
		backgroundTextura = carregarTextura(renderer,"../img/creditos2.png");
	}else
		backgroundTextura = carregarTextura(renderer,"../img/creditos2.png");
...
int pausado(){
	...
	botao = Mix_LoadWAV( "../music/beep.wav" );
			...
			SDL_QueryTexture(retomarTexto,NULL,NULL,&retomarRect.w,&retomarRect.h);
			 ...
			 // código para o botão jogar novamente
			//surface com codificacao utf-8 para aceitar acento
			SDL_Surface *reiniciarSurface = TTF_RenderUTF8_Solid(font,"Jogar Novamente",color);
