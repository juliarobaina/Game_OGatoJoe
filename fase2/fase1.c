...
int fase1(){
	...
	iniciarSinais(&sinalR,&sinalRPos);
	...
	iniciarInimigo2(&inimigo2,&sRect_inimigoPos2,&dRect_inimigoPos2,aleatorio);
	...
	int contador = 600;//contador recebe o total de segundos que eu quero que o timer faça; 60 = 1 min, 180 = 3min
		...
		if(PAUSADO){
			...
			if(VIDASGATO > 0){
				...
				if(GATOUP){
					...
					neal(&catJoePosicao.loadRect,&caixaBlocoRacao1.loadRect,catJoePosicao.loadRect,caixaBlocoRacao1.loadRect,
